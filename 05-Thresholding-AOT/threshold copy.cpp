...
int main(int argc, char **argv) {
    ...
    switch (method.get()) {
        case Thresh::BINARY:
            result(x, y) = u8(select(input(x, y) >= threshold, 0, 255)); 
            break;
        case Thresh::BINARY_INV:
            result(x, y) = u8(select(input(x, y) >= threshold, 0, 255));
            break;
        case Thresh::TO_ZERO:
            result(x, y) = u8(select(input(x, y) >= threshold, input(x, y), 0)); 
            break;
        case Thresh::TO_ZERO_INV:
            result(x, y) = u8(select(input(x, y) >= threshold, 0, input(x, y))); 
            break;
        default:
